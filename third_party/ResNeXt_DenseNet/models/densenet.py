...
class DenseNet(nn.Module):
  ...
  def forward(self, x):
    if type(x) == list:
        ...
        out_ori = self.trans1(self.dense1(out_ori))
        out_ori = self.trans2(self.dense2(out_ori))
        out_ori = self.dense3(out_ori)
        out_ori = torch.squeeze(F.avg_pool2d(F.relu(self.bn1(out_ori)), 8))
