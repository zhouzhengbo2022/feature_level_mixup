  ...
  def __init__(self, num_classes):
    ...
    for m in self.modules():
      if isinstance(m, nn.Conv2d):
        ...
        m.bias.data.zero_()
    #self.se = SELayer(3,16)
    self.cbam = CBAM(n_channels_in = 192, reduction_ratio = 1, kernel_size = 1)
  def forward(self, x):
    if type(x) == list:
        out_list = []
        ...
        for num in range(3):
            ...
            if num==0:
                mix = torch.zeros_like(output)
            #mix += ws[num] * output
            #x = self.classifier(x)
        out_all = torch.stack([out_list[0],out_list[1],out_list[2]],2).cuda().squeeze(4)
        out_all = self.cbam(out_all)
        #print(out_all.shape)
        ...
        for i in results:
            i = i.squeeze(2)
            #if num == 0:
            #    mix = torch.zeros_like(i)
            num += 1
            mix += i.view(i.size(0), -1)
        x[-1] = x[-1].cuda()
